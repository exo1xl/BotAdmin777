...
async def leave1(ctx):
    voice31 = get(bot.voice_clients, guild = ctx.guild)
    if voice31 and voice31.is_connected:    
        await VoiceClient.disconnect(voice31)
...
async def join(ctx):
    ...
    await vc.connect()
    await ctx.send(f'Bot connected to { vc }')
@bot.command(pass_context=True)
async def play(ctx, query):
    song_there = os.path.isfile('song.mp3')
    url2 = f'ytsearch:{ query }'
    voice = get(bot.voice_clients, guild = ctx.guild)
    if voice and voice.is_connected:
        while True:
            await ctx.send(f'Bot connected { voice }')
            try:
                if song_there:
                    os.remove('song.mp3')
            ...
            try:
                ...
                with youtube_dl.YoutubeDL(ydl_opts) as ydl:
                    print('Загружаю')
                    player = ydl.download([url2])
                    await songs.put(player)
                    for file in os.listdir('./'):
                        if file.endswith('.mp3'):
                            name = file
                            print(f'переименовываю: { name } ')
                            os.rename(file, 'song.mp3')
            except PermissionError:
                print('error')
            voice.play(discord.FFmpegPCMAudio('song.mp3'), after=leave1)
        ...
        while False:
            await voice.connect
            await ctx.send(f'Bot connected to { voice }')
        try:
            if song_there:
                    os.remove('song.mp3')
                    await ctx.send("Wait please")
        ...
        try:
            ...
            with youtube_dl.YoutubeDL(ydl_opts) as ydl:
                ...
                await ctx.send('Ooops... I am caught a problem. Enter (/fix)')
        voice.play(discord.FFmpegPCMAudio('song.mp3'), after=leave1)
        voice.source = discord.PCMVolumeTransformer(voice.source)
        voice.source.volume = vol
...
async def stop(ctx):
    ...
    await voice3.stop()
    await ctx.send("Player has been stoped")
    if voice3 and voice3.is_connected:    
        await VoiceClient.disconnect(voice3)
@bot.command(pass_context=True)
@has_permissions(administrator=True)
async def pause(ctx):
...
async def leave(ctx):
    ...
    if voice3 and voice3.is_connected:    
        await VoiceClient.disconnect(voice3)
@bot.command(pass_context=True, aliases=['que', 'p'])
